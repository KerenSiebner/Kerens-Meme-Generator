...
function getMemes(inputTxt='') {
    if (!inputTxt) return gImgs
    else{
        gFilterByWord += inputTxt
        console.log('gFilterByWord', gFilterByWord)
        const imgsIncludeWord = gImgs.filter(img => img.keywords.includes(`${gFilterByWord}`))
        return imgsIncludeWord
    }
}